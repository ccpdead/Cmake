...
main (int argc, char** argv)
{
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud (new pcl::PointCloud<pcl::PointXYZ>);//存放加载的点云信息
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered (new pcl::PointCloud<pcl::PointXYZ>);//保存曲率滤波后的点云信息
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_radius (new pcl::PointCloud<pcl::PointXYZ>);//保存曲率滤波后的点云信息
...
////////////////////////////////////////////////////
  ...
  outrem.setInputCloud(cloud_p2);       //设置输入点云
  outrem.setRadiusSearch(0.2);       //设置半径为0.8的范围内找临近点
  outrem.setMinNeighborsInRadius(50); //设置查询点的邻域点集数小于2的删除
  // apply filter
  outrem.filter(*cloud_radius); //执行条件滤波   在半径为0.8 在此半径内必须要有两个邻居点，此点才会保存
////////////////////////////////////////

  //创建惯性矩阵估计对象,设置输入点晕,进行运算
